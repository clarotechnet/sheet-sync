          ...
          <div style={{ height: Math.max(400, barData.length * 30 + 100) }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={barData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                ...
                <Tooltip
                //cor do ticket com os dados no grafico.
                  contentStyle={{
                ...
                />
                <Legend
                  wrapperStyle={{ color: 'hsl(223 16% 70%)', fontSize: 12 }}
                />
                <Bar dataKey="pendente" name="Pendente" fill="#f59e0b" radius={[4, 4, 0, 0]}>
                  <LabelList dataKey="pendente" position="top" fill="hsl(223 16% 70%)" fontSize={10}
                    formatter={(v: number) => v > 0 ? v : ''} />
                </Bar>
                <Bar dataKey="confirmada" name="Confirmada" fill="#22c55e" radius={[4, 4, 0, 0]}>
                  <LabelList dataKey="confirmada" position="top" fill="hsl(223 16% 70%)" fontSize={10}
                    formatter={(v: number) => v > 0 ? v : ''} />
                </Bar>
                <Bar dataKey="cancelada" name="Cancelada" fill="#ff0909" radius={[4, 4, 0, 0]}>
                  <LabelList dataKey="cancelada" position="top" fill="hsl(223 16% 70%)" fontSize={10}
                    formatter={(v: number) => v > 0 ? v : ''} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
